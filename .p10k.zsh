# Generated by Powerlevel10k configuration wizard on 2026-04-18 at 15:57 CDT.
...
() {
  ...
  function prompt_vpn() {
    local ip profile prefix

    ip=$(ip -4 addr show tun0 2>/dev/null | awk '/inet /{print $2}' | cut -d/ -f1)

    if systemctl is-active --quiet openvpn@sp; then
      prefix="SP"
    elif systemctl is-active --quiet openvpn@htb; then
      prefix="HTB"
    else
      prefix=""
    fi
    ...
    if [[ -n "$ip" && "$prefix" ]]; then
      p10k segment -f 205 -i "$prefix" -t "${ip}"
    else
      p10k segment -f 244 -i 'X' -t "No VPN"
    fi
  }

  prompt_promptnote() {
    [[ -z ${_PN-} ]] || p10k segment -f 208 -i "*" -t $_PN
  }
